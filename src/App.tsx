...
function App() {
    ...
    if(e.target instanceof HTMLButtonElement){
      if(action === ACTION.DRAW || action === ACTION.FILL){
        setCanvas((prevCanvas)=>{
          ...
          newCanvas[rowIndex][colIndex] = mouseClick === MOUSE.LEFT_CLICK ? color1 : color2
          return newCanvas
        })
      }else{
        setCanvas((prevCanvas)=>{
  ...
  function handleMouseDown(e:React.MouseEvent<HTMLButtonElement>,rowIndex:number,colIndex:number){
  ...
  function handleFill(e:React.MouseEvent<HTMLButtonElement>,rowIndex:number,colIndex:number){
    if(action !== ACTION.FILL){
      return;
    }
    if(e.button === MOUSE.RIGHT_CLICK){
      setCanvas((prevCanvas)=>floodFill(prevCanvas,rowIndex,colIndex,color2))
    }else{
      setCanvas((prevCanvas)=>floodFill(prevCanvas,rowIndex,colIndex,color1))
    }
  }
  ...
  return (
    <>
      ...
      <ButtonPanel action={action} handleEraseAll={handleEraseAll} handleAction={handleAction}/>
    </>
  )
}
